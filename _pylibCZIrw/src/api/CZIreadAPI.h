...
private:
	...
	std::shared_ptr<libCZI::ISubBlockCache>                     spSubBlockCache; ///< The pointer to the subblock cache object, may be null (in which case no caching is done)
	SubBlockCacheOptions                                        subBlockCacheOptions; ///< Options for using the subblock cache
...
public:
	...
	/// Creates a spReader and spAccessor (SingleChannelTilingScalingAccessor) for the
	/// czi document pointed by the given filepath.
    /// This constructor allows defining a subblock cache to be used for performance optimization.
    /// \param  fileName                Filename of the file.
    /// \param  subBlockCacheOptions    Options for initializing the subblock cache.
	CZIreadAPI(const std::wstring& fileName, const SubBlockCacheOptions& subBlockCacheOptions);
    ...
    /// \param  fileName                Filename (or URI) of the file (the interpretation of the string is stream class specific).
    /// \param  subBlockCacheOptions    Options for initializing the subblock cache.
	CZIreadAPI(const std::string& stream_class_name, const std::wstring& fileName, const SubBlockCacheOptions& subBlockCacheOptions);
	...
	/// Returns information about the current state of the subblock cache. If caching is not active, the returned struct will contain zeros.
    /// <returns>A SubBlockCacheInfo struct containing the cache information.</returns>
