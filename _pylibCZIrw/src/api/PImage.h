...
private:
  std::shared_ptr<libCZI::IBitmapData>
      ptrBitmapData; ///< The pointer to the IBitMapData object.
  void *ptrData; ///< The pointer to the first (top-left) pixel of the bitmap.
  std::size_t
      stride; ///< The stride of the bitmap data (pointed to by `ptrData`).
...
public:
  ...
  libCZI::PixelType get_pixelType() const {
    return this->ptrBitmapData->GetPixelType();
  }
  ...
  std::vector<size_t> get_shape() const {
    return {get_height(), get_width(), GetNChannels()};
  }
