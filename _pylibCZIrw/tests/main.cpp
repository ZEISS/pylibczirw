...
void mem_test_read(std::wstring filepath, std::string coordinates,
                   std::wstring scenefilter) {
  ...
  auto bitmap = czi.GetSingleChannelScalingTileAccessorData(
      pixeltype, roi, bgColor, 1.0, coordinates, scenefilter);
...
int main(int argc, char *argv[]) {
  if (argc != 4) {
    ...
    std::cerr << "Program called with insufficient or excess arguments!"
              << std::endl;
    exit(0);
  }
