...
PYBIND11_MODULE(_pylibCZIrw, m)
{
    py::class_<CZIreadAPI>(m, "czi_reader", py::module_local())
        ...
        .def(py::init([](const std::string& stream_class_name, const std::wstring& fileName) {
            return new CZIreadAPI(stream_class_name, fileName);
        }))
        .def(py::init([](const std::wstring& fileName, const SubBlockCacheOptions& subBlockCacheOptions) {
            return new CZIreadAPI(fileName, subBlockCacheOptions);
        }))
        .def(py::init([](const std::string& stream_class_name, const std::wstring& fileName, const SubBlockCacheOptions& subBlockCacheOptions) {
        ...
        .def("GetSingleChannelScalingTileAccessorData", [](
                    ...
                    libCZI::IntRect roi,
                    libCZI::RgbFloatColor bgColor,
                    float zoom,
                    const std::string& coordinateString,
                    const std::wstring& SceneIndexes
                ){
                    // We release the GIL to make data access parallelizable via python threads.
                    // Note: Whenever the executed C++ code tries to access python objects,
                    //       the GIL must be re-acquired using py::gil_scoped_acquire and released again
                    //       to make the rest of the code parallelizable again.
                    // For more details see: https://pybind11.readthedocs.io/en/stable/advanced/misc.html#global-interpreter-lock-gil
                    py::gil_scoped_release release;
                    auto result = self.GetSingleChannelScalingTileAccessorData(pixeltype, roi, bgColor, zoom, coordinateString, SceneIndexes);
                    return result;
                }
            )
    ...
    py::class_<libCZI::ISubBlockCache::PruneOptions>(m, "PruneOptions", py::module_local())
        .def(py::init<>())
        .def_readwrite("maxMemoryUsage", &libCZI::ISubBlockCache::PruneOptions::maxMemoryUsage)
        .def_readwrite("maxSubBlockCount", &libCZI::ISubBlockCache::PruneOptions::maxSubBlockCount);

    py::class_<SubBlockCacheOptions>(m, "SubBlockCacheOptions", py::module_local())
        .def(py::init<>())
        .def_readwrite("cacheOnlyCompressed", &SubBlockCacheOptions::cacheOnlyCompressed)
