...
public:
    ...
    /// \param  scaleX              The length of a pixel in x-direction in the unit meters. If unknown/invalid, this value is numeric_limits<double>::quiet_NaN().
    /// \param  scaleY              The length of a pixel in y-direction in the unit meters. If unknown/invalid, this value is numeric_limits<double>::quiet_NaN().
    /// \param  scaleZ              The length of a pixel in z-direction in the unit meters. If unknown/invalid, this value is numeric_limits<double>::quiet_NaN().
    /// \param  channelNames        A dictionary of channels, where key is the channel index and value is the corresponding channel name.
    /// \param  customAttributes    A dictionary of custom attributes, where the key is the name of the attribute and value is the corresponding custom attribute value.
    /// \param  displaySettings     A dictionary of display settings, where key is channel number and value is the corresponding display setting.
	void WriteMetadata(const std::wstring& documentTitle, std::optional<double> scaleX, std::optional<double> scaleY, std::optional<double> scaleZ, const std::map<int, std::string>& channelNames, const std::map<std::string, const libCZI::CustomValueVariant>& customAttributes, const std::map<int, const ChannelDisplaySettingsStruct>& displaySettings);
